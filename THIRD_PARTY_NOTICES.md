...
- 服务提供方：小米 MiMo 平台
- 当前接入模型：`mimo-v2.5-tts-voiceclone`
- 调用方式：通过 HTTPS API 上传文本和参考音频，返回生成音频。

注意事项：

- MiMoTTS 是云 API，不属于本项目 MIT License 的一部分。
- 使用 MiMoTTS 时，用户输入的文本和参考音频会发送到 MiMo API 服务。
- 用户需要自行准备、保管并遵守 MiMo API Key、平台服务条款、计费规则、限流规则和模型使用规范。
- 本项目只在本机后端读取 `MIMO_API_KEY`，前端只显示是否已配置，不会返回 API Key 明文。
