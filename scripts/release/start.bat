...
set "BOOT_LOG=%~dp0logs\start_bat.log"
echo ======================================== > "%BOOT_LOG%"
echo tts-video Release start.bat >> "%BOOT_LOG%"
echo Time: %DATE% %TIME% >> "%BOOT_LOG%"
echo Root: %~dp0 >> "%BOOT_LOG%"

set "PS1=%~dp0scripts\release\start_release.ps1"
if not exist "%PS1%" set "PS1=%~dp0start_release.ps1"
...
if not exist "%PS1%" (
  echo [ERROR] start_release.ps1 was not found.
...
echo.
if "%EXIT_CODE%"=="0" (
  echo tts-video exited.
) else (
  echo Startup failed. Check the console message or logs\start_bat.log and logs\start_release.log.
)
echo.
pause
